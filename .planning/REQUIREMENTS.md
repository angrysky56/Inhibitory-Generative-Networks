...
- [ ] **SUB-02**: Implement 2D grid Continuous-Time Recurrent Neural Network (CTRNN) with local connectivity.
...
- [ ] **SCU-02**: Implement clipping/thresholding to maintain state boundaries post-inhibition.
- [ ] **SCU-03**: Integrate 3-layer forward pass mapping input target to sculpted output grid.

### Evaluation
- [ ] **EVAL-01**: Implement metric for signature-matching (IGN vs target distribution).
- [ ] **EVAL-02**: Implement comparative benchmark against a baseline additive generative model.
